...
func TestRun_Success(t *testing.T) {
	...
	err := r.Run("echo @msg", map[string]string{"msg": "hello"}, false, false)
	if err != nil {
...
func TestRun_ErrorFromCommand(t *testing.T) {
	...
	r := &runner.CommandRunnerImpl{Executor: ex}
	err := r.Run("ls @opt", map[string]string{"opt": "-z"}, false, false)
...
func TestRun_DryRun(t *testing.T) {
	...
	r := &runner.CommandRunnerImpl{Executor: ex}
	err := r.Run("echo @a @b", map[string]string{"a": "one", "b": "two"}, true, false)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if called {
...
func TestRun_MultiplePlaceholders(t *testing.T) {
	...
	r := &runner.CommandRunnerImpl{Executor: ex}
	err := r.Run("echo @first @second", map[string]string{"second": "two", "first": "one"}, false, false)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
